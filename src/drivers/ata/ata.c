...
/* ata.c - Minimal 28-bit LBA PIO driver (read-only) for Wind/Tempest
 *
 * Supports primary IDE channel (I/O base 0x1F0, control 0x3F6) in PIO mode.
 * Only implements sector reads which is sufficient for our EXT2 driver.
 */
...
int
    ata_pio_read (uint64_t lba, uint32_t count, void *buf)
{
        if ( count == 0 || buf == NULL )
                return -1;
        ...
        for ( uint32_t i = 0; i < count; i++ )
        {
                ...
                outb(ATA_PRIMARY_IO + ATA_REG_DRIVE, 0xE0 | ((cur_lba >> 24) & 0x0F));
