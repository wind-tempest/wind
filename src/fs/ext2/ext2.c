    ...
    ext2_mount (uint64_t base_lba)
{
        ...
        memcpy(&g_superblock, sb_raw, sizeof(ext2_superblock_t));

        if ( g_superblock.magic != EXT2_SUPER_MAGIC )
        {
#ifdef EXT2_DEBUG
                printf("EXT2: bad magic 0x%04x (expected 0x%04x)\n",
                       g_superblock.magic,
                       EXT2_SUPER_MAGIC);
#endif
        ...
        {
#ifdef EXT2_DEBUG
                printf("EXT2: unsupported block size (log=%u)\n", g_superblock.log_block_size);
#endif
...
#ifdef EXT2_DEBUG
        printf("EXT2: mounted. Block size %u, groups %u\n", g_block_size, groups);
#endif
        return EXT2_OK;
}
        ...
        while ( tok )
        {
                ...
                for ( int i = 0; i < 12 && found_ino == 0; i++ )
                {
                        ...
                        while ( off < g_block_size )
                        {
                                ...
                                if ( ent->rec_len == 0 )
                                        break;
                                if ( ent->inode != 0 && ent->name_len == strlen(tok)
                                     && memcmp(ent->name, tok, ent->name_len) == 0 )
                                {
                                        found_ino = ent->inode;
                                        break;
                                }
    ...
    read_inode (uint32_t ino, ext2_inode_t *out)
{
        ...
        if ( bytes_first >= need )
        {
                ...
                memcpy(out, tmp + off_in_block, need);
        }
        else
        {
                /* Copy first portion */
                memcpy(out, tmp + off_in_block, bytes_first);
                ...
                memcpy((uint8_t *) out + bytes_first, tmp, need - bytes_first);
        }
...
int
    ext2_list_root (ext2_list_cb_t cb)
{
        if ( !cb )
                return EXT2_ERR_INVALID;
    ...
    ext2_read (ext2_file_t *file, void *buf, size_t len)
{
        ...
        (void) len;
        /* ----------------------------------------------------------------------
         *  Simple file read (direct blocks only, no sparse nor indirect)
         * ------------------------------------------------------------------*/
        if ( !file || !buf )
                return EXT2_ERR_INVALID;
        ...
        while ( remaining > 0 )
        {
                ...
                if ( chunk > remaining )
                        chunk = remaining;
                memcpy(dst, block_buf + off_in_block, chunk);
                dst += chunk;
                file->pos += (uint32_t) chunk;
                remaining -= chunk;
        }
