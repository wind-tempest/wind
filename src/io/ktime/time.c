    ...
    get_bios_time (struct bios_time *time)
{
        ...
        if ( is_bcd )
        {
                ...
                time->month  = bcd_to_decimal(month);
                time->year   = (uint16_t) (bcd_to_decimal(year) + (bcd_to_decimal(century) * 100));
                time->day_of_week = bcd_to_decimal(day_of_week);
        }
        else
        {
                time->second      = seconds;
    ...
    get_date_string (char *buffer, size_t buffer_size)
{
        ...
        /* Format: DD-MM-YYYY */
        snprintf(buffer, buffer_size, "%02d/%02d/%04d", time.day, time.month, time.year);
}
    ...
    get_datetime_string (char *buffer, size_t buffer_size)
{
        ...
        printf("%02d-%02d-%04d %02d:%02d", time.day, time.month, time.year, time.hour, time.minute);
...
const char *
    get_day_of_week_string (uint8_t day_of_week)
{
