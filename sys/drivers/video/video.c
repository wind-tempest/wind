	...
	for ( kuint32_t y = 0; y < fb_info.height; y++ )
		{
			...
			if ( fb_info.bpp == 16 )
				{
					volatile kuint16_t *row =
					    (volatile kuint16_t *) row_base;
				...
				{
					volatile kuint32_t *row =
					    (volatile kuint32_t *) row_base;
					for ( kuint32_t x = 0; x < fb_info.width; x++ )
						{
							row[x] = color;
						}
